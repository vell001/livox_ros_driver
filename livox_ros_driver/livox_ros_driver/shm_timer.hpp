...
namespace shm_timer
{
    ...
    void open()
    {
        ...
        int ok = ftruncate(fd, sizeof(lidar_time_mem));
        if (ok){
            printf("shm_timer ftruncate failed!!!\n");
            return;
        }
        lidar_time_mem_data = (lidar_time_mem *)mmap(NULL, sizeof(lidar_time_mem), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        if (!lidar_time_mem_data)
        {
            printf("shm_timer mmap failed!!!\n");
            return;
        }
        memset(lidar_time_mem_data, 0, sizeof(lidar_time_mem));
    }
    ...
    void recv_lidar_packet(uint64_t raw_stamp)
    {
        if (!lidar_time_mem_data)
        {
            open();
